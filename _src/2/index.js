...
export default class PlanetarySystem {
    ...
    constructor (selector, { camera, canvas, renderBody, orbits }) {
        ...
        function renderOrbits ($parent, orbit) {
            $parent.appendChild(orbit.$clone);
            orbit.objects.forEach(object => {
                orbit.$clone.appendChild(object.$clone);
                object.orbits.forEach(orbit => renderOrbits(object.$clone, orbit));
            })
        }

        function renderBodies ($parent, orbit) {
            $parent.appendChild(orbit.$el);
            orbit.objects.forEach(object => {
                object.$body && object.$el.appendChild(object.$body);
                orbit.$el.appendChild(object.$el);
                object.orbits.forEach(orbit => renderBodies(object.$el, orbit));
            })
        }
